...
exports.main = function(options, callbacks) {
	...
	/*Snippet: enable/disable console.log*/
	...
	pagemods.PageMod({
		...
		contentScriptFile: [data.url("jquery-1.5.2.min.js"), data.url("subWin.js"), data.url("loginChecker.js")],
	...
	var debridWidget = require("widget").Widget({
		...
		panel: panels.Panel({
			...
			contentScriptFile: [data.url("jquery-1.5.2.min.js"),data.url("popup.js"),data.url("loginChecker.js")],
			onShow: function(){
				this.postMessage({'type':'loading'});
							
				hiddenFrames.add(hiddenFrames.HiddenFrame({
							  onReady: function() {
								this.element.contentWindow.location = DM_ROOT + "/?lang=en";
								let self = this;
			...
			onMessage: function(m){
				switch (m){
					case "hidePanel":
						...
						break;
					case "openLoginTab":
						require("tabs").open(DM_ROOT + "login.php" + "?lang=en");
						break;
					case "openSubWin":
